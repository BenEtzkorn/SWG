$(document).ready(function () {
    ...
    $('#editDVDButton').click(function (event) {
        ...
        $.ajax({
           ...
           crossDomain: true,
           type: 'PUT',
           url: 'http://localhost:61407/dvd/' + $('#editDVDId').val(),
           dataType: 'json',
           processData: false,
           data: JSON.stringify({
             dvdId: $('#editDVDId').val(),
    ...
    $('#searchButton').click(function(){
      ...
      resetAll();
        ...
        if(choice == "showAll"){
        var tempurl='http://localhost:61407/dvds';
        }
        else if(choice == "searchTitle" && term != null){
          var tempurl='http://localhost:61407/dvds/title/' + term;
        }
        else if(choice == "searchYear" && term != null && term.length == 4 && isNaN(term) == false){
          var tempurl='http://localhost:61407/dvds/year/' + term;
        }
        else if(choice == "searchDirector" && term != null){
        var tempurl='http://localhost:61407/dvds/director/' + term;
        }
        else if(choice == "searchRating" && term != null){
        var tempurl='http://localhost:61407/dvds/rating/' + term;
        }
        else{
        //  error: function() {
...
function showEditForm(dvdId) {
    ...
    $.ajax({
        type: 'GET',
        url: 'http://localhost:61407/dvd/' + dvdId,
        success: function(data, status) {
